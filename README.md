# Decimal-Calculator
This is a library written in D which implements functions for you to perform calculations with the decimal number system rather than the binary number system. The idea is to bypass the precision loss that happens as a result of the binary system, with this library you can perform FP math with a total precision of 20 decimal places or more (you can change the precision at will), even for numbers that are very big or very tiny.

In order to use it you only need to download this library into your project's folder and import it in your code.
...
It is very fast, running on my "AMD Ryzen 5 2600X" CPU it was able to perform an average of 214k additions per sec, 154k subtractions per sec, 292k multiplications per sec, 38k divisions per sec, 677k modulos per sec, 79k powers per sec and 95 roots per sec, all with a precision of 20 digits, very impressive. I know the roots are much slower than the other operations, but it is still quite fast.
